...
# Loop through each sheet and export to its mapped JSON filename
for sheet_name, df in all_sheets.items():
    print(f"\nProcessing sheet: {sheet_name}")
    df = df.fillna("")  # Replace NaN with empty strings
    data = df.to_dict(orient="records")

    # Get the proper file name from the mapping
    file_name = sheet_name_map.get(sheet_name, sheet_name.lower())
