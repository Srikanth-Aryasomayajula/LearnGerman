...
.then(arrayBuffer => {
  ...
  // 🔍 Log rows that contain the "φ" character (probably from wrong ß encoding)
  const suspectRows = allData.filter(row =>
    Object.values(row).some(val => typeof val === "string" && val.includes("φ"))
  );
  if (suspectRows.length > 0) {
    console.warn("Rows containing φ character (possible ß issue):", suspectRows);
  } else {
    console.log("No φ characters found.");
  }
